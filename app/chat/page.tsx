  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gradient-primary overflow-hidden">
      ...
      <div className="flex-1 flex flex-col w-full h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto bg-gradient-to-br from-[#0f1f14] via-[#228B22]/10 to-[#4CBB17]/5">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center h-full px-2 sm:px-4 py-8">
              <h2 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-gradient-primary mb-8 sm:mb-10 font-mono animate-float">
                What can I help with?
              </h2>

              <div className="flex items-center gap-2 sm:gap-3 border-2 border-[#4CBB17] rounded-2xl px-3 sm:px-4 md:px-5 py-2 sm:py-3 bg-[#1a3a1a]/60 w-full max-w-[95%] sm:max-w-[700px] md:max-w-[950px] shadow-lg shadow-[#228B22]/40 hover:shadow-xl hover:shadow-[#228B22]/60 transition-all glass-morphism backdrop-blur-md animate-slide-in-up">
                <input
                  type="text"
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  ...
                  disabled={isLoading}
                  className="flex-1 text-sm sm:text-base outline-none text-[#b8e6b8] placeholder-[#228B22]/60 bg-transparent disabled:opacity-50"
                />
              ...
              <div className="mt-8 sm:mt-12 flex flex-wrap justify-center gap-2 sm:gap-3">
                {suggestedPrompts.map((prompt, idx) => (
                  <button
                    ...
                    onClick={() => setInputValue(prompt)}
                    disabled={isLoading}
                    className="border-2 border-[#4CBB17] rounded-full px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm text-[#4CBB17] hover:bg-[#228B22]/40 hover:text-white transition disabled:opacity-50 animate-slide-in-up shadow-md hover:shadow-lg hover:shadow-[#228B22]/40 transform hover:scale-105"
            ...
            <div className="px-3 sm:px-6 py-4 sm:py-8">
              ...
              <div className="space-y-4 sm:space-y-6">
                {messages.map((message, idx) => (
                  <div 
                    key={message.id} 
                    className={`group relative max-w-[90%] sm:max-w-[80%] ${message.type === 'user' ? 'ml-auto' : ''} animate-slide-in-up`}
                  ...
                  >
                    ...
                    <div
                      className={`p-4 sm:p-5 rounded-2xl shadow-lg transition transform hover:scale-102 ${
                        message.type === "user"
                          ? "bg-gradient-to-br from-[#228B22] to-[#4CBB17] text-white rounded-br-none shadow-[#228B22]/50 hover:shadow-[#4CBB17]/60 hover:shadow-xl"
                    ...
                    >
                      <p className="whitespace-pre-wrap text-sm sm:text-base leading-relaxed font-medium">{message.content}</p>
                    </div>
                ...
                {isLoading && (
                  <div className="flex items-center gap-3 text-[#4CBB17] max-w-[90%] sm:max-w-[80%] animate-fade-in">
        ...
        {messages.length > 0 && (
          <div className="border-t-2 border-[#4CBB17]/50 bg-gradient-to-r from-[#0f1f14] via-[#228B22]/20 to-[#4CBB17]/10 p-2 sm:p-4 flex items-center gap-2 sm:gap-3 flex-shrink-0 shadow-lg shadow-[#228B22]/40 backdrop-blur-sm glass-morphism animate-slide-in-up">
            <input
              ...
              placeholder="Message Soft GPT AI..."
              disabled={isLoading}
              className="flex-1 text-sm sm:text-base outline-none text-[#b8e6b8] placeholder-[#228B22]/60 bg-transparent px-2 sm:px-3 disabled:opacity-50"
            />
            <Paperclip className="w-5 h-5 text-[#4CBB17] cursor-pointer hover:text-[#228B22] hidden sm:block transition animate-glow" />
            <button
              onClick={handleSendMessage}
